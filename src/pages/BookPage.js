...
function BookPage() {
  ...
  const [newBook, setNewBook] = useState({ name: '', publicationYear: '', stock: '', authorId: '', publisherId: '' });
    ...
    <div style={containerStyle}>
      ...
      <div style={formStyle}>
        ...
        <input
          ...
          style={inputStyle}
        />
        <button onClick={addBook} style={buttonStyle}>Ekle</button>
      </div>
        ...
        <tbody>
          {books.map(book => (
            <tr key={book.id}>
              ...
              <td style={tdStyle}>{book.publisherId}</td>
              <td style={tdStyle}>
                <button style={actionButtonStyle} onClick={() => setEditBook(book)}>Güncelle</button>
                <button style={{ ...actionButtonStyle, backgroundColor: '#e74c3c' }} onClick={() => deleteBook(book.id)}>Sil</button>
              </td>
            </tr>
          ))}
        </tbody>
      ...
      {editBook && (
        <div style={formStyle}>
          ...
          <input
            ...
            onChange={e => setEditBook({ ...editBook, authorId: e.target.value })}
            style={inputStyle}
          />
          <input
            placeholder="Yayınevi ID"
            value={editBook.publisherId}
            onChange={e => setEditBook({ ...editBook, publisherId: e.target.value })}
            style={inputStyle}
          />
          <button onClick={updateBook} style={buttonStyle}>Güncelle</button>
        </div>
      )}
    </div>
  );
