    ...
    <div style={containerStyle}>
      ...
      <ul style={listStyle}>
        {publishers.map(publisher => (
          <li key={publisher.id} style={listItemStyle}>
            {publisher.name} - {publisher.establishmentYear} - {publisher.address}
            <div>
              <button style={buttonStyle} onClick={() => setEditPublisher(publisher)}>Güncelle</button>
              <button style={{ ...buttonStyle, backgroundColor: '#e74c3c' }} onClick={() => deletePublisher(publisher.id)}>Sil</button>
            </div>
          </li>
        ))}
      </ul>
      <div style={formStyle}>
        ...
        <input
          ...
          style={inputStyle}
        />
        <button onClick={addPublisher} style={submitButtonStyle}>Ekle</button>
      </div>
      {editPublisher && (
        <div style={formStyle}>
          ...
          <input
            ...
            onChange={e => setEditPublisher({ ...editPublisher, establishmentYear: e.target.value })}
            style={inputStyle}
          />
          <input
            placeholder="Adres"
            ...
            style={inputStyle}
          />
          <button onClick={updatePublisher} style={submitButtonStyle}>Güncelle</button>
        </div>
      )}
    </div>
  );
